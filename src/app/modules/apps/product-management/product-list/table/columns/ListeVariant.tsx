...
const ListeVariant: React.FC<Props> = ({ variantes, id }) => {
  ...
  const [userForEdit] = useState <User>({
    smallPrice : variantes && variantes[0]?.price ? variantes[0]?.price : 0,
    mediumPrice : variantes && variantes[1]?.price ? variantes[1]?.price : 0,
    grandePrice : variantes && variantes[2]?.price ? variantes[2]?.price : 0,
    smallQuantity : variantes && variantes[0]?.quantity ? variantes[0]?.quantity : 0,
    mediumQuantity : variantes && variantes[1]?.quantity ? variantes[1]?.quantity : 0,
    grandeQuantity : variantes && variantes[2]?.quantity ? variantes[2]?.quantity : 0,
    ...
    onSubmit: async (values, {setSubmitting}) => {
      ...
      const variantss = [
        ...
        },
        {
          ...
          id: variantes && variantes[1]?.id ? variantes[1]?.id : '',
          size: "M"
        },
        {
          quantity: values.grandeQuantity,
          price: values.grandePrice,
          id: variantes && variantes[2]?.id ? variantes[2]?.id : '',
          size: "G"
        }
      ];
