...
const updateCategory = (category: any,id: ID): Promise<User | undefined> => {
  return axios
  .post(`${process.env.REACT_APP_API_URL2}/api/update_category/${id}`, category,{      headers: { 'Content-Type': 'application/x-www-form-urlencoded' }
})
  .then((response: AxiosResponse<Response<User>>) => response.data)
    .then((response: Response<User>) => response.data)
}
...
export {deleteSelectedCategory,getCategories,createCategory,deleteCategory,getCategoryById,updateCategory}
